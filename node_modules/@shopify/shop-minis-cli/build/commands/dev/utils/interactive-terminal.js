...
export class InteractiveTerminal {
    ...
    async startApp(platform, deviceId) {
        try {
            ...
            let buildType = this.options.buildType;
            let snapshotVersion = this.options.snapshotVersion;
            outputDebug(`Start app with CLI options: ${JSON.stringify(this.options)}`);
            if (!snapshotVersion && buildType === BuildType.Release) {
                snapshotVersion =
                    platform === Platform.Android ? '2.208.0+103023' : '2.208.0+103029';
                outputDebug(`No snapshot version provided, using default ${platform} snapshot version ${snapshotVersion}`);
